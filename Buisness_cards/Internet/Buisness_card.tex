...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\foreach \i in {0,1} \foreach \j in {0,1,2,3,4} {
			...
			\node at (\i*\cardw+0.2\cardw,\j*\cardh+0.5\cardh) {\includegraphics[width=0.3\cardw]{site_qrcode.png}};

			\node[black!25!gray] at (\i*\cardw+0.50\cardw,\j*\cardh+0.85\cardh) {\Large \textbf{Francesco Rombaldoni}};
			
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.65\cardh) {\textbf{Potenzia}};
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.59\cardh) {\textbf{la tua}};
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.53\cardh) {\textbf{connessione}};
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.47\cardh) {\textbf{internet}};
			\node at (\i*\cardw+0.50\cardw,\j*\cardh+0.2\cardh) {-Tel-};
			\node at (\i*\cardw+0.50\cardw,\j*\cardh+0.1\cardh) {f.rombaldoni@campus.uniurb.it};
			
			\node at (\i*\cardw+0.85\cardw,\j*\cardh+0.5\cardh) {\includegraphics[width=0.26\cardw]{internet.png}};
		};
	\end{tikzpicture}
