...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\foreach \i in {0,1} \foreach \j in {0,1,2,3,4} {
			...
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.65\cardh) {\textbf{Stampa}};
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.59\cardh) {\textbf{di piccoli}};
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.53\cardh) {\textbf{oggetti}};
			\node at (\i*\cardw+0.53\cardw,\j*\cardh+0.47\cardh) {\textbf{in 3D}};
			\node at (\i*\cardw+0.50\cardw,\j*\cardh+0.2\cardh) {-Tel-};
			\node at (\i*\cardw+0.50\cardw,\j*\cardh+0.1\cardh) {f.rombaldoni@campus.uniurb.it};
			
			\node at (\i*\cardw+0.85\cardw,\j*\cardh+0.5\cardh) {\includegraphics[width=0.26\cardw]{pokemon.png}};
		};
	\end{tikzpicture}
